      ...
      <div className="max-w-2xl mx-auto">
            ...
            >
              {editingId === todo.id ? (
                ...
                <div className="flex flex-1 gap-2">
                  <input 
                    className="flex-1 bg-slate-800 text-white px-3 py-1 rounded-lg border border-indigo-500 focus:outline-none"
                    value={editText} 
                    onChange={(e) => setEditText(e.target.value)}
                    autoFocus
                  />
                  <button onClick={saveEdit} className="bg-green-600 p-2 rounded-lg text-white">Done</button>
                </div>
              ) : (
                /* Standard UI */
                <>
        ...
        {todos.length > 0 && (
          <div className="mt-8 flex justify-between items-center text-sm text-slate-500 px-4">
            ...
            <button 
              onClick={() => { if(confirm('Clear all?')) setTodos([]); }}
